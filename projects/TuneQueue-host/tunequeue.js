...
$(document).ready(function () {
    //For added protection, the firebase api key and shared playlist id are provided through url search parameters rather than hardcoded.
    ...
    //store firebase api key and playlist id for use after host is redirected for authentication
    if (!localStorage.getItem('firebaseApiKey') || !localStorage.getItem('playlistID')) {
    ...
    function addTrackToPlaylist(trackID) {
        $.ajax({
            type: 'POST',
            url: 'https://api.spotify.com/v1/playlists/' + localStorage.getItem('playlistID') + '/tracks?uris=spotify%3Atrack%3A' + trackID,
